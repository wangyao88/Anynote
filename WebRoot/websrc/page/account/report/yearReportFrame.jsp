...
<html>
         ...
         Ext.onReady(function() {
               // 工具栏
			    var todoListToolbar = new Ext.Toolbar({
			    	renderTo: 'yearReportToolBarDiv',
			    	items: [
			    	    new Ext.Button({
						    id: 'year-cast-data-button',
							text: '年支出数据列表',
							iconCls: 'folder_export'
						}),
						 new Ext.Button({
						    id: 'year-import-data-button',
							text: '年收入数据列表',
							iconCls: 'pictures'
						})
					]
			   ...
			   // 年支出数据列表按钮
				$("#year-cast-data-button").click(function(){
					    // 全局参数
					    ...
					    var accountListStore = new Ext.data.JsonStore({
					        url: '<%=path%>/accountAction.do?method=queryYearCastReportData',
					        root: 'datas',
				...
				// 年支出数据列表按钮
				$("#year-import-data-button").click(function(){
					    // 全局参数
					    var baseParams = {start:0, limit:"<%=Constants.PAGE_SIZE %>",fromDate:"<%=DateUtils.getFirstDayOfMonthAsString(DateUtils.DATE_PATTON_1)%>", toDate:"<%=DateUtils.getLastDayOfMonthAsString(DateUtils.DATE_PATTON_1)%>"};
		                // 账目数据源
					    var accountListStore = new Ext.data.JsonStore({
					        url: '<%=path%>/accountAction.do?method=queryYearImportReportData',
					        root: 'datas',
					        totalProperty: 'results',
					        fields: ['accountId', 'accountBookId', 'accountBookName', 'accountType', 'categoryName', 'accountDateStr', 'money', 'remark', 'updateDateStr'],
					        baseParams: baseParams,
               ...
               var panel = new Ext.Panel({
		                id:'year-report-main-panel',
		                baseCls:'x-plain',
		                renderTo: 'yearReportDiv',
		                layout:'table',
		                anchor:'100%',
		                ...
		                items:[{
		                    ...
		                    colspan: 2,
		                    width:1120,
		                    html: '<iframe src="<%=path%>/websrc/page/account/report/yearReportLine.jsp" width="1150px" height="545px;" scrolling="no" style="margin:0px;" frameborder="0"></iframe>'
		                }]
		            });
  ...
  <body>
       <div id="yearReportToolBarDiv"></div>
       <div id="yearReportDiv" style="width:100%;background-image:url('<%=path%>/websrc/image/login/login-bg.jpg');background-size:cover;">
       </div>
  </body>
</html>
